...
Packages live in `packages/{core,system,apps,extra}/`.
...
CRDs are in `api/v1alpha1/` and `api/apps/v1alpha1/`.
App Kinds (like `Postgres`, `Kafka`) are registered dynamically at runtime from `ApplicationDefinition` resources — they are not static CRDs.
...
- `*.tgz` files — Helm chart archives, expected in the repo.
