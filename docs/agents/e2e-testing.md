...
- Conservative escalation: edits to `packages/library/`, `packages/core/`, `api/`, `cmd/`, `internal/`, top-level `hack/*.sh|*.bats` helpers, the shared `hack/e2e-chainsaw/_lib/`, the chainsaw config `hack/e2e-chainsaw/.chainsaw.yaml`, the `Makefile`, or the E2E workflows escalate to the **full suite**. A per-suite edit (any file under `hack/e2e-chainsaw/<app>/`) selects **only** that suite, with one exception: a **switched-off** suite, whose `chainsaw-test.yaml` has been renamed to `chainsaw-test.yaml.disabled` (the `hack/e2e-chainsaw/backup/` demo), defines no runnable suite and its files are ignored entirely. Any other directory under `hack/e2e-chainsaw/` that carries no `chainsaw-test.yaml` — a shared fixtures directory, or a suite nested deeper than the discovery scan looks — escalates to the **full suite** instead, because Chainsaw discovers suites recursively and selecting nothing for it would skip E2E.
- Second escalation rule, and in practice the one that fires most: a changed package under `packages/{apps,system,extra}/` whose dependency graph reaches no existing Chainsaw suite runs the **full suite**, because nothing narrower would exercise it at all. Many `packages/system/` components are in this class. Coverage is decided **per changed path**, so a single uncovered path escalates the whole run however narrowly the other paths resolve; deciding it from the merged selection instead would let any other path that contributes a suite name swallow the escalation without a trace. A path owned by several PackageSources counts as covered when **any one** of them reaches a suite — `system/postgres-operator` belongs both to `cozystack.monitoring`, which reaches none, and to `cozystack.postgres-operator`, which reaches postgres and harbor, and a change to it selects those two rather than escalating.
...
When adding a new app package, confirm `select-e2e.sh` maps it correctly (it has a unit test, `hack/select-e2e_test.bats`). A new app whose suite directory does not yet exist is not quietly skipped — it reaches no suite, so every diff touching it runs the full suite until `hack/e2e-chainsaw/<app>/chainsaw-test.yaml` is in place. Suite names come from `src_to_suites()`, the inverse of `select-install.sh`'s `suite_to_source()`: a source named neither `<suite>-application` nor `<suite>` needs an explicit entry in both, or its suite becomes unreachable and every change to its package escalates.
