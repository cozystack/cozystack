...
- `CACHE_TAG` — the tag of the mode=max build cache each image reads (and writes under `WRITE_CACHE=1`). Defaults to `buildcache`. CI never builds both architectures in one job: images are built for amd64 and, on native arm64 runners, again for arm64 under `<tag>-arm64` with `CACHE_TAG=buildcache-arm64`. The talos and testing packages are amd64 only and get no arm64 build. On main and release lines, `hack/stitch-multiarch.sh` then joins each pair into one index and republishes the packages artifact pinned on the index digests. It skips any image whose digest is also pinned in a file it does not rewrite, so the kamaji control-plane provider, built for both, still ships amd64 only until its gzipped manifest is handled (see `docs/agents/image-refs.md`).
