...
* **[bucket] Fix S3 Manager endpoint mismatch with COSI-issued credentials**: The S3 Manager deployment was constructing its S3 endpoint from the tenant namespace host, while `BucketAccess` secrets are issued for the actual SeaweedFS S3 endpoint. This mismatch caused "invalid credentials" errors when users tried to log in with valid credentials from the bucket secret. The deployment now resolves the endpoint from `BucketInfo`, matching the same source used for credentials, and falls back to the constructed namespace host during first-time deploys before access secrets exist ([**@IvanHunters**](https://github.com/IvanHunters) in #2211, #2215).

* **[api] Fix spurious OpenAPI post-processing errors for non-apps group versions**: The API server no longer logs false errors while generating OpenAPI specs for core and other non-`apps.cozystack.io` group versions. The post-processor now exits early when the base `Application` schemas are absent, reducing noisy startup logs without affecting application schema generation ([**@kvaps**](https://github.com/kvaps) in #2212, #2217).
