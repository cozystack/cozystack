...
- [ ] The PR body says `✅ RC full e2e was verified`; if it says the gate was bypassed, obtain explicit maintainer acceptance or add `e2e/full` and wait for that opt-in run.
...
- [ ] **The published rc passed mandatory full E2E.** Confirm the promote PR says `✅ RC full e2e was verified`; the normal promote PR itself runs no E2E. Add `e2e/full` manually only when a repeat is required, and treat a ⚠️ bypass line as an explicit release-risk decision.
