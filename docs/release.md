...
Step 1 covers the labeled work: [`backport-audit`](../cmd/backport-audit/README.md) resolves each `kind/backport` / `kind/backport-previous` PR to the line the label meant **at merge time**, not from the label text, then reports each one as already on the branch, `pending` on an open backport PR, `dropped` with the reason someone recorded, or `MISSING`. It reproduces whichever rule the bot was running when the PR merged — the branch enumeration described in [The backport bot](#the-backport-bot) from 2026-08-03, `getLatestRelease` before it — so a PR labeled during a freeze window is audited against the line being stabilised, exactly as the bot targeted it. Listing the labeled PRs with `gh pr list --search "is:merged label:kind/backport"` is not a substitute — it says nothing about whether the change reached the branch, which is the only thing that matters at this point in the cut. Steps 2 and 3 remain necessary because they cover what step 1 structurally cannot see: changes that were never labeled at all.
