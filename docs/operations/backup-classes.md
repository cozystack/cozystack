...
## Postgres: a restored copy converges its application passwords only after leaving bootstrap
...
The consequence for a **restore into a copy** (a differently-named target): the recovered database keeps the source roles with their source password hashes, but the target chart generates fresh random passwords into the target's `<release>-credentials`, and the role-reconciling init-job is skipped while `bootstrap.enabled` is set on the restored app. Until the target leaves bootstrap — clear `bootstrap.enabled` on the Postgres app, or run a later user-driven `helm upgrade` — the passwords advertised in `<target>-credentials` do not match the recovered roles, so a client reading that Secret cannot log in as an application user. The data is intact and reachable as the CNPG superuser; only the chart-managed application logins lag, and they converge (the init-job runs `ALTER ROLE … WITH PASSWORD` onto the target Secret's values) as soon as bootstrap is cleared. After a to-copy restore, converge deliberately rather than assuming the advertised application password works immediately. The credentials Secret is flagged `postgres.cozystack.io/credentials-pending: "true"` and the chart NOTES print the same warning while bootstrap is on. An in-place restore is unaffected only as long as `passwordRotation` has not been bumped since the backup was taken: bump the counter, take no new backup, restore in place, and the Secret now carries the post-rotation passwords while the recovered roles carry the pre-rotation ones — the same mismatch, converging the same way once bootstrap is cleared. Automating this convergence (clearing `bootstrap.enabled` once recovery is healthy) is tracked as follow-up.

## MariaDB: a restored copy re-applies application passwords late, and root stays desynced

MariaDB has the mirror of the Postgres restore caveat, plus a sharper edge on `root`. The `strategy.backups.cozystack.io/MariaDB` driver takes a logical dump that includes `mysql.global_priv` (the grant table, password hashes and all) — mariadb-operator only excludes it when `ignoreGlobalPriv` is set, and that field defaults to `false` for a replication (non-Galera) instance, which is what this chart runs. So a restore into a copy replays the source's grants onto the target, while the target chart has generated fresh random passwords into `<target>-credentials`. Application users recover, but only on the operator's `--requeue-sql` tick (up to ~11h) rather than promptly, because the Secret content itself did not change so the `k8s.mariadb.com/watch` label does not fire. `root` never recovers on its own: `rootPasswordSecretKeyRef` is applied only at datadir bootstrap and the rotation counter deliberately skips `root`, so the password a tenant reads for `root` from `<target>-credentials` stays wrong from the restore onward. Rotate the application users (bump `passwordRotation`) after a to-copy restore to realign them promptly, and treat the advertised `root` password as unreliable on a restored copy. Making the target's own grants authoritative by setting `ignoreGlobalPriv: true` on the shipped `cozy-default-mariadb` strategy is tracked as follow-up.
