...
| Kafka           | `S3_ENDPOINT` env on the strategy Job | full URL (scheme preserved); the Job's `curl --aws-sigv4` prepends `https://` if the endpoint carries no scheme, keeps the whole base URL and honours `backupStorage.forcePathStyle` when building the object URL |
