...
>
> - **CloudNativePG operator bumped to 1.28.1** (`postgres-operator` chart). A minor operator upgrade for the whole fleet; the CNPG CRDs move with it. This also fixes a latent bug where the operator image was newer than its bundled CRDs (`status.instanceID.sessionID` pruned), which made the operator believe the instance manager restarted and fail **every** backup cluster-wide with `instance manager was restarted during backup`. Note that Helm does not upgrade CRD schemas on `helm upgrade`, so on existing clusters the regenerated CNPG CRDs must be applied out of band (`kubectl apply`); a fresh install picks them up.
> - **LINSTOR scheduler admission webhook removed** (`linstor-scheduler` bumped to the upstream release that drops it). That webhook decoded every pod through a stale Kubernetes API and stripped `initContainers[].restartPolicy` cluster-wide, which broke the plugin's restartable barman sidecar (and other native sidecars). Consequence beyond Postgres: pods are **no longer auto-assigned `spec.schedulerName: linstor-scheduler`**, so LINSTOR data-locality scheduling becomes advisory for every stateful workload — the default scheduler places pods and DRBD serves I/O over the network. Workloads that require data locality must now set `schedulerName: linstor-scheduler` explicitly.
>
