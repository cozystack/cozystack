...
Migration 43 is fixed to match the `-system` suffix, and migration 45 re-runs the hand-over for clusters that already ran the hardcoded version. Both are pre-upgrade hooks, so they land before `<name>-system` re-renders. Audit anyway — a Cluster already deleted cannot be recovered by either:
...
| `<name>-system` | *(none)* | **At risk.** Migration 45 hands it over on the next upgrade. Do not reconcile `<name>-system` before the migration runs. |
