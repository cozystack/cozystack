...
Removal is lazy rather than global, which is worth knowing before relying on it as a clean-up. The reconciler only runs for namespaces an active Package still targets, so disabling removes the LimitRange from those and leaves it behind in any namespace whose Package has since been removed or disabled. On an operator restart the initial full reconcile covers every currently-targeted namespace, so the ordinary disable flow does what it looks like; an orphaned namespace keeps its ceiling until something targets it again, and the leftover object carries `app.kubernetes.io/managed-by: cozystack-package-controller` if you want to find and delete them by hand.
...
The same delay applies to any downward move, including lowering the knob itself. A namespace currently defaulting more than the new value is indistinguishable, on the object, from one sitting at a ceiling the operator raised, so it is held and then lowered on the same schedule rather than immediately. Reconciles are event-driven, so the drop lands on the first one after the grace period has elapsed rather than at the moment it does — a lowered limit reaches previously-raised namespaces no sooner than the grace period, and no later than the next Package reconcile after it. The countdown is visible on the object while it runs:

```bash
kubectl get limitrange cozystack-system-defaults -A \
  -o custom-columns=NS:.metadata.namespace,DEFAULT:.spec.limits[0].default.memory,'CLEAR-SINCE:.metadata.annotations.operator\.cozystack\.io/memory-ceiling-clear-since'
```
