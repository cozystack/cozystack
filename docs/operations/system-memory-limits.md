...
The same delay applies to lowering the knob itself. A namespace currently defaulting more than the new value is indistinguishable, on the object, from one sitting at a ceiling the operator raised, so it is held and then lowered on the same schedule rather than immediately — expect a lowered limit to reach previously-raised namespaces within the grace period, not on the next reconcile. The countdown is visible on the object while it runs:
