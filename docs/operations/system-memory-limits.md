...
Both CEL blocks on this page are the shipped defaults at that tag, `DefaultOOMCgroupRankingExpression` and `DefaultOOMTriggerExpression` in `pkg/machinery/constants/constants.go`; cross-check them against the constant at the tag rather than against a rendered configuration reference. The rendered `OOMConfig` reference does build its expressions from those same constants, through the `examples:` annotation on the document type, but what it renders is one example document rather than a table of defaults: it re-serialises the expressions with different parenthesisation, and it sets `sampleInterval: 100ms` against a `DefaultOOMSampleInterval` of `500ms`, with nothing marking which of its fields are shipped defaults and which are illustration.
...
Prefer fixing the workload generating the pressure, once you have established there is one. Persistent triggering has more than one cause and the trigger context in `oomactions` is what distinguishes them. The first clause weighs only `System` and `Podruntime` pressure, and Talos puts kubelet and the CRI containerd under `podruntime`, so a node whose kubelet or container runtime is squeezed keeps triggering with no pod anywhere near its own ceiling. Static pods are not in that set: kubelet gives them pod cgroups under `kubepods/` exactly as it does any other pod, the three pod classes carry no weight in either of the first clause's vectors, and a class absent from a weight vector contributes nothing to the product. Pressure from a static pod can therefore only reach the second clause, the cause-blind global backstop that one cgroup reclaiming against its own limit can equally drive on its own. Read the context first, and raise a limit only where a cgroup sitting at its ceiling is what the numbers actually show.
