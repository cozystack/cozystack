...
cozy_wait_linstor_pool_free() {
  ...
  _deadline=$(( $(date +%s) + _timeout ))
  while :; do
    # blockstor exposes each storage pool as a cluster-scoped StoragePool CR
    # (blockstor.cozystack.io) whose status.freeCapacity is the KiB the
    # satellite reports — the same figure LINSTOR's `sp l` free_capacity
    # carried, so no controller exec is needed (external mode has no
    # in-cluster linstor-controller). Filter to ZFS variants (both `ZFS` and
    # `ZFS_THIN`) so DISKLESS placeholders and any future non-ZFS driver are
    # skipped, and guard against pools that have not reported yet (freeCapacity
    # absent on OFFLINE satellites; the schema marks it optional) — without the
    # null guard `sort -n` would rank the string "null" ahead of real numbers
    # and the loop would silently poll to timeout. Emit
    # `<free_capacity_kib>:<node>` lines so a single sort yields the smallest
    # pool and its owner in one round-trip.
    _min_line=$(kubectl get storagepools.blockstor.cozystack.io -o json 2>/dev/null |
      jq -r '.items[]
        | select(((.spec.providerKind // "") | test("^ZFS")) and (.status.freeCapacity != null))
        | "\(.status.freeCapacity):\(.spec.nodeName)"' 2>/dev/null |
      sort -n | head -n 1) || _min_line=""
    _min_kib="${_min_line%%:*}"
    _min_node="${_min_line#*:}"
    if [ -n "$_min_kib" ] && [ "$_min_kib" -ge "$_min_free_kib" ] 2>/dev/null; then
      echo "» blockstor ZFS pool free: smallest satellite ${_min_node} has $(( _min_kib / 1024 / 1024 )) GiB (>= ${_min_free_gib} GiB threshold)"
      return 0
    fi
    if [ "$(date +%s)" -ge "$_deadline" ]; then
      echo "» WARNING: blockstor ZFS pool free did not reach ${_min_free_gib} GiB on every satellite within ${_timeout}s (smallest observed: ${_min_kib:-unknown} KiB on ${_min_node:-unknown}); continuing (next test may face zfs create out-of-space)" >&2
      kubectl get storagepools.blockstor.cozystack.io -o wide 2>&1 | sed 's/^/  blockstor-pool: /' >&2 || true
      return 1
    fi
    sleep 5
  done
