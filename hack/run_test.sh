#!/bin/sh
###############################################################################
#  run_test.sh — POSIX-sh test-runner с live-trace и корректным RC-выходом    #
###############################################################################
set -eu

TEST_FILE=${1:?Usage: ./run_test.sh <file.bats> [pattern]}
PATTERN=${2:-*}
LINE='----------------------------------------------------------------'
...
run_one() {
  ...
  {
    (
      PS4='+ '           # префикс для set -x
      set -eu -x         # strict + trace
      "$fn"
    )
    printf '__RC__%s\n' "$?"     # маркер с итоговым кодом
  } 2>&1 | tee "$log" | while IFS= read -r line; do
        case "$line" in
          '__RC__'*) : ;;                        # маркер не выводим
          '+ '*)   cmd=${line#'+ '}
