...
EOF
  ...
  timeout 120 sh -ec "until kubectl -n tenant-test get hr ${name} >/dev/null 2>&1; do sleep 2; done"
  kubectl -n tenant-test wait hr ${name} --timeout=10m --for=condition=ready
  kubectl -n tenant-test wait hr ${name}-system --timeout=10m --for=condition=ready
  timeout 120 sh -ec "until kubectl -n tenant-test get deploy -l app.kubernetes.io/instance=${name}-system -o jsonpath='{.items[0].status.readyReplicas}' | grep -q '1'; do sleep 5; done"
