...
dump_diagnostics() {
  ...
  kubectl -n "$TEST_NAMESPACE" get etcdcluster,etcdbackup,pvc,backupjobs,restorejobs,backups -o wide 1>&2 || true
  kubectl -n "$TEST_NAMESPACE" describe backupjob "$TEST_BACKUPJOB_NAME" 1>&2 || true
  kubectl -n "$TEST_NAMESPACE" describe restorejob "$TEST_RESTOREJOB_INPLACE" 1>&2 || true
