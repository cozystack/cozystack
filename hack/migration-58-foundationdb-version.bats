#!/usr/bin/env bats
# -----------------------------------------------------------------------------
# Unit tests for platform migration 58 --> 59 (carry a FoundationDB
...
# (hack/testdata/migration-58-foundationdb/) and pin that:
...
#
# Run with: hack/cozytest.sh hack/migration-58-foundationdb-version.bats
# -----------------------------------------------------------------------------

FAKEBIN="$PWD/hack/testdata/migration-58-foundationdb"
MIG="$PWD/packages/core/platform/images/migrations/migrations/58"
...
@test "carries each supported line over and leaves everything else alone" {
  ...
  echo "$annotation" | grep -q 'cozystack.io/migration-58-needs-attention='
