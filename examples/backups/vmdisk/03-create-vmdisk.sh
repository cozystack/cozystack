...
log_info "Imports the Ubuntu Noble cloud image into a 5Gi ${VMI_DISK_STORAGE_CLASS:-replicated} disk"
...
spec:
  ...
  storage: 5Gi
  # Default 'replicated' for a real DRBD cluster; overridable so the CI lane can
  # pass its own class (the PR gate runs DRBD-off and only renders 'local', so a
  # hardcoded 'replicated' never binds a PVC and the import hangs).
  storageClass: ${VMI_DISK_STORAGE_CLASS:-replicated}
EOF
...
kubectl -n "$NAMESPACE" wait hr vm-disk-backup-src --for=condition=ready --timeout="${IMPORT_WAIT:-600}s"
wait_for_field datavolume.cdi.kubevirt.io vm-disk-backup-src \
    '{.status.phase}' Succeeded "$NAMESPACE" "${IMPORT_WAIT:-600}"
