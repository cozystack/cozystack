...
# Default 1800s: the Velero CSI data mover copies the whole disk (5Gi) to S3.
# Overridable (BACKUP_WAIT) so the CI harness can bound it under the Chainsaw op
# timeout. Fail fast if the job flips to Failed instead of polling out the budget.
wait_for_field backupjob vmdisk-backup '{.status.phase}' Succeeded "$NAMESPACE" "${BACKUP_WAIT:-1800}" Failed
