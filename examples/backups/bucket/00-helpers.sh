...
# cluster; the mirror Job reuses it to invoke `s3-mirror`. Looks the deployment
# up by its `app` label, not by the image string, so a dev image with an
# arbitrary name (e.g. a ttl.sh build) is still found.
controller_image() {
  kubectl get deploy -A -l app=backupstrategy-controller \
