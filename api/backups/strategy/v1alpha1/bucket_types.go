...
type BucketTemplate struct {
	...
	// subcommand; the default strategy sets it to the controller image. It must
	// be a literal reference: unlike the other fields it is consumed unrendered by
	// the Backup-cleanup Job (which has no application to template against), so a
	// template expression here would delete-wedge every Backup on an
	// InvalidImageName pod.
