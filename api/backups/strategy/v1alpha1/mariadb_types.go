...
// Exactly one of S3, PersistentVolumeClaim or Volume must be set. The XValidation
// rule enforces that at admission time so a misconfigured strategy is rejected
// up-front instead of silently picking S3 first (the order buildMariaDBBackupStorage
// happens to walk).
// +kubebuilder:validation:XValidation:rule="(has(self.s3) ? 1 : 0) + (has(self.persistentVolumeClaim) ? 1 : 0) + (has(self.volume) ? 1 : 0) == 1",message="exactly one of s3, persistentVolumeClaim or volume must be set"
...
//
// The strategy template is rendered via internal/template, which walks every
// string field in the JSON-marshalled object and substitutes Go-template
// expressions. Both Name and Key are therefore templatable - tenants can
// vary either by .Application / .Parameters.
type MariaDBSecretKeySelector struct {
	...
	// supported (the template engine walks every string in the marshalled
	// template, see MariaDBSecretKeySelector godoc).
	Key string `json:"key"`
}
...
type MariaDBS3TLS struct {
	...
	// (e.g. cozystack's seaweedfs-s3). Templating is supported on both
	// Name and Key (see MariaDBSecretKeySelector godoc).
	// +optional
