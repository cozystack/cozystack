...
type ConfigSpec struct {
	...
	// Stream the gateway VM's guest serial console into a `guest-console-log` container beside virt-launcher, readable with `kubectl logs`, and install the bring-up diagnostics that print into it. The only view of a router that stalls inside the guest before the VyOS management API answers, when the VMI reports Running with an address but nothing in the cluster can say how far the boot got. The console alone carries kernel and boot output; the emitter (`files/guest-diag.sh`, run once a minute from `/etc/cron.d` and silent after the first 20 minutes of uptime) adds cloud-init status, failed units, whether the seeded config committed, whether nginx — the `:443` listener — is up, the IPsec state, and the load, which is what separates a guest that is still working from one that is wedged. The two are one switch because the emitter is pointless without the capture and the capture is far weaker without the emitter. Off by default, and three costs are why. It overrides the platform's cluster-wide `disableSerialConsoleLog` (packages/system/kubevirt), which is set because that container has been seen holding virt-launcher in `PodInitializing` (kubevirt/kubevirt#15989) — that would take the gateway down rather than explain it, so check the virt-launcher Pod reaches Running after enabling it. It is part of the VM's device list, so changing it on a live gateway restarts the VM and drops the tunnel with it. And the emitter half rides the cloud-init seed, which re-runs only when the firmware UUID changes, so enabling this on an already-booted gateway gets you the console but not the extra fields until the VM's next first boot (change `cloudInitSeed` to force one). It buys visibility into the next boot, not the current one.
	// +kubebuilder:default:=false
