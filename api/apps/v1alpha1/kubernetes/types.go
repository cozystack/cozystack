...
type ConfigSpec struct {
	...
	// Pod and Service address ranges of the tenant cluster. Change the defaults when they overlap a network the cluster has to reach, such as a VPN peer or another cluster this one talks to by Pod address.
	// +kubebuilder:default:={}
...
type Network struct {
	// IPv4 range Pods take their addresses from.
	// +kubebuilder:default:="10.243.0.0/16"
	PodCIDR string `json:"podCIDR"`
	// IPv4 range ClusterIP Services take their addresses from. The cluster DNS Service takes the tenth address of the range, so the prefix must be /24 or wider.
	// +kubebuilder:default:="10.95.0.0/16"
