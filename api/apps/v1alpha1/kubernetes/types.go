...
type Talos struct {
	...
	// Talos image-factory schematic ID. Defaults to the cozystack-tested vanilla schematic. Operators using custom schematics (system extensions, kernel args) override here. A node group can override it for itself via `nodeGroups.<name>.schematicID`. Constrained to `^[A-Za-z0-9._:-]+$`, applied to the effective per-group value: the ID is interpolated into the worker machine config, which is applied through a shell heredoc. A 64-hex factory digest and the readable path an operator chose on a self-hosted factory or mirror both satisfy it.
