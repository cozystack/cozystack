...
// Secrets labelled “cozystack.io/ui=true”. Each data key is a separate object.
...
func (r *REST) Get(ctx context.Context, name string, opts *metav1.GetOptions) (runtime.Object, error) {
	...
	// We need to identify secret name and key. Iterate secrets in namespace with UI label
	// and return the matching composed object.
	list, err := r.core.Secrets(ns).List(ctx, metav1.ListOptions{LabelSelector: labels.Set{uiLabelKey: uiLabelValue}.AsSelector().String()})
	if err != nil {
		return nil, err
	}
...
func secretKeyToObj(sec *corev1.Secret, key string, val []byte) *corev1alpha1.TenantSecretsTable {
	...
	return &corev1alpha1.TenantSecretsTable{
		...
		ObjectMeta: metav1.ObjectMeta{
			...
			Annotations:       filterUserLabels(sec.Annotations),
		},
...
func filterUserLabels(m map[string]string) map[string]string {
	...
	for k, v := range m {
		if k == uiLabelKey || strings.HasPrefix(k, "internal.cozystack.io/") {
			continue
		}
		out[k] = v
	}
