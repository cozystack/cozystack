...
// between them was the rationale for the api-side parity work that
// followed PR #2509.
...
func TestConvertApplicationToHelmRelease_PerAppTimeoutOverridesGlobal(t *testing.T) {
	r := &REST{
		...
		releaseConfig: config.ReleaseConfig{
			...
			HelmInstallTimeout:        30 * time.Minute,
		},
	}
	app := &appsv1alpha1.Application{
	...
	if hr.Spec.Install.Timeout == nil || hr.Spec.Install.Timeout.Duration != 30*time.Minute {
		t.Errorf("Install.Timeout = %v, want 30m (per-app override)", hr.Spec.Install.Timeout)
	}
	if hr.Spec.Upgrade.Timeout == nil || hr.Spec.Upgrade.Timeout.Duration != 30*time.Minute {
		t.Errorf("Upgrade.Timeout = %v, want 30m (per-app override)", hr.Spec.Upgrade.Timeout)
	}
}
