...
// TestWatchedTypes pins what SetupWithManager watches. The builder
// exposes nothing about its watches, so a dropped one leaves both Go
// suites green while the behaviour it carried silently degrades to the
// periodic resync — for TenantGateway that means the wildcard private
// key sits in a namespace that stopped terminating TLS for up to
// sourceResyncInterval.
func TestWatchedTypes(t *testing.T) {
	s := newScheme(t)
	want := map[string]bool{"Secret": false, "Namespace": false, "TenantGateway": false}
	for _, obj := range watchedTypes() {
		gvk, err := apiutil.GVKForObject(obj, s)
		if err != nil {
			t.Fatalf("GVK for %T: %v", obj, err)
		}
		seen, expected := want[gvk.Kind]
		if !expected {
			t.Errorf("unexpected watched kind %s — a new watch needs its own reason here", gvk.Kind)
			continue
		}
		if seen {
			t.Errorf("kind %s watched twice", gvk.Kind)
		}
		want[gvk.Kind] = true
	}
	for kind, seen := range want {
		if !seen {
			t.Errorf("%s is not watched; its events would only reach the reconciler on the next resync", kind)
		}
	}
}
