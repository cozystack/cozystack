...
// its upload proxy is exposed. A namespace owning a Gateway counts as one
// only while something there still terminates TLS: a TenantGateway carries
// certMode=edge when the class it selected ends TLS at the provider's edge,
// and a namespace whose Gateways are all at the edge has no consumer for the
// replica, so it does not get one. That is a statement about the REPLICA and
// not about the namespace being key-free — see ownsTerminationPoint for what
// stays behind. Because the source is derived from the same values that make
// the consumers reference it, the replica is created whenever the consumers
// expect it — no manual labelling, and no window where a child controller
// references a Secret that will never exist. Clearing
// _cluster.wildcard-secret-name (disabling the feature) is the only path that
// prunes EVERY replica; moving a tenant onto an edge-terminated class prunes
// that tenant's replica alone. A values channel that is absent or unreadable,
// or a source Secret that is merely missing or mistyped, leaves existing
// replicas in place — so a transient gap, a delete+recreate rotation, or a
...
func (r *Reconciler) targetNamespaces(ctx context.Context, sourceNS string, copyToUploadProxy bool) ([]string, error) {
	...
	for i := range list.Items {
		...
		if terminates || copyToUploadProxy && ns.Name == uploadProxyNamespace {
...
// watchedTypes returns the object kinds whose events enqueue the config
// key, in the order SetupWithManager registers them:
//
//   - Secret — replica self-heal and feature enable/disable. The
//     manager's Secret cache is scoped (SecretCacheByObject) to managed
//     replicas plus the values channel, so this watch only ever delivers
//     those two.
//   - Namespace — a tenant gains or loses a termination point.
//   - TenantGateway — a namespace stops being a termination point the
//     moment its TenantGateway moves onto an edge-terminated class, and
//     that object is what this controller reads to decide. Without the
//     watch the withdrawal waits for the periodic resync, leaving the
//     wildcard private key in a namespace that no longer serves it for
//     up to sourceResyncInterval.
//
// The operator source Secret is deliberately absent: its name is dynamic
// so the cache cannot scope it, and its rotation is picked up by the
// result requeue in Reconcile.
//
// The set is a function rather than a chain of builder calls because the
// builder exposes nothing about what it watched, so a dropped watch is
// invisible to a test that goes through SetupWithManager.
func watchedTypes() []client.Object {
	return []client.Object{
		&corev1.Secret{},
		&corev1.Namespace{},
		&gatewayv1alpha1.TenantGateway{},
	}
}

// SetupWithManager wires the reconciler as a singleton: every event from
// every type in watchedTypes maps to the one config key.
func (r *Reconciler) SetupWithManager(mgr ctrl.Manager) error {
	toSingleton := handler.EnqueueRequestsFromMapFunc(enqueueConfigKey)
	b := ctrl.NewControllerManagedBy(mgr).Named("wildcardsecret")
	for _, obj := range watchedTypes() {
		b = b.Watches(obj, toSingleton)
	}
	return b.Complete(r)
}
