...
const (
	...
	// BackupContentTypeLogical mirrors the operator enum value used on
	// MariaDB.spec.bootstrapFrom.backupContentType. The driver only
	// produces logical Backups today.
	BackupContentTypeLogical = "Logical"

	// BackupKind is the kind reference used in
	// MariaDB.spec.bootstrapFrom.backupRef.
	BackupKind = "Backup"
)
...
// k8s.mariadb.com/MariaDB CR. Unknown fields are preserved on the server.
...
// BootstrapFrom mirrors the operator's MariaDB.spec.bootstrapFrom shape.
// Restore-on-bootstrap is the canonical mariadb-operator restore primitive;
// the driver patches this on the live (in-place) or target (to-copy)
// MariaDB CR to drive a restore.
type BootstrapFrom struct {
	BackupRef          *BackupReference `json:"backupRef,omitempty"`
	BackupContentType  string           `json:"backupContentType,omitempty"`
	TargetRecoveryTime string           `json:"targetRecoveryTime,omitempty"`
}

// BackupReference points at a Backup or PhysicalBackup CR in the same
// namespace as the MariaDB CR being restored.
