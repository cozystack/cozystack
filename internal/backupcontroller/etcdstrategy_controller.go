...
// EtcdCluster.spec.bootstrap.restore.source. Prefers the persisted
// snapshot (authoritative); falls back to driverMetadata for older
// artefacts that pre-date the snapshot field.
//
// For S3, the BackupJob path persists the strategy-rendered "key" (a
// prefix-only fragment like "etcd/"), because the operator's
// backup_job.go appends "<backupName>.db" to it at backup time
// (internal/controller/factory/backup_job.go: s3Key = strings.TrimRight
// (s3.Key, "/")+"/"+backupName+".db"). The restore-agent reads S3_KEY
// verbatim and downloads exactly that object - giving it the prefix
// alone would return a 0-byte directory marker and abort the bootstrap.
// resolveEtcdRestoreDestination reconstructs the operator's filename
// convention here so the restore-agent fetches the actual snapshot.
func resolveEtcdRestoreDestination(backup *backupsv1alpha1.Backup, snap *etcdBackupSnapshot) (etcdtypes.EtcdBackupDestination, bool) {
	...
	if snap != nil && snap.Destination.PVC != nil {
		return etcdtypes.EtcdBackupDestination{
			PVC: &etcdtypes.EtcdBackupPVC{
				ClaimName: snap.Destination.PVC.ClaimName,
				SubPath:   snap.Destination.PVC.SubPath,
			},
		}, true
	}
...
func validateRenderedEtcdDestination(d strategyv1alpha1.EtcdDestinationTemplate) error {
	s3, pvc := d.S3, d.PVC
	if (s3 == nil) == (pvc == nil) {
		return errors.New("destination must specify exactly one of s3 or pvc")
	}
	if s3 != nil {
		if s3.Bucket == "" {
	...
	if pvc != nil && pvc.ClaimName == "" {
		return errors.New("destination.pvc.claimName is empty after templating")
	}
	return nil
}
...
// two types share JSON fields verbatim, so this is a pure shape-cast.
func strategyToEtcdBackupDestination(d strategyv1alpha1.EtcdDestinationTemplate) (etcdtypes.EtcdBackupDestination, error) {
	...
	out := etcdtypes.EtcdBackupDestination{}
	if d.S3 != nil {
		var fps *bool
	...
	if d.PVC != nil {
		out.PVC = &etcdtypes.EtcdBackupPVC{
