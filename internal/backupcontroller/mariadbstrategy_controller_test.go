...
func TestMarshalMariaDBBackupSnapshot_RoundTrip(t *testing.T) {
	...
	raw, err := marshalMariaDBBackupSnapshot(newMariaDBApp("mariadb-src", "tenant"), rendered, parameters)
	if err != nil {
...
func TestCreateMariaDBBackupArtifact_ArtifactShape(t *testing.T) {
	...
	sourceApp := newMariaDBApp("src", "tenant")
	...
	t.Run("S3 storage populates Artifact with URI", func(t *testing.T) {
		...
		artefact, err := r.createMariaDBBackupArtifact(context.Background(), job, resolved, mdbBackup, rendered, sourceApp)
		if err != nil {
