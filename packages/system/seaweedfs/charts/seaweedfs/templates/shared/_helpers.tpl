...
{{- /* normalise both sides: the API server canonicalises quantities, so a raw
       compare fires on a respelling that changed nothing. */}}
...
{{- /* both sides are strings from an include, so compare them as numbers: a
       lexicographic gt reads 10Gi as smaller than 5Gi and skips the patch. */}}
{{-               if gt (float64 $newSize) (float64 $oldSize) }}
{{-                 $commands = append $commands (printf "kubectl patch pvc %s-%s-%s-%d -p '{\"spec\":{\"resources\":{\"requests\":{\"storage\":\"%v\"}}}}'" $dir.name $seaweedfsName $volumeName $e $desiredSize) }}
{{-               end }}
{{-             end }}
{{-           end }}
{{-         end }}
