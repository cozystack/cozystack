...
The transferred volume is handed to its `VMDisk` without being copied: the `PersistentVolume` is retained and re-bound into the claim the disk expects. `Retain` is held for the swap only. Between deleting the transferred claim and re-pointing the volume's `claimRef`, the volume is `Released`, and under `Delete` the provisioner would reclaim it on the spot. Once the new claim and the volume both report `Bound` to each other, the transferred claim is gone, and CDI has adopted the claim into the `VMDisk`'s DataVolume as its sole owner, the controller sets the volume back to the reclaim policy its StorageClass declares (`Delete` when the class names none or no longer exists). Only then does it create the `VMInstance` and mark the VM `Succeeded`, so a task in `Creating` a little longer than the transfer itself is this step waiting on the PV controller and CDI. From then on an imported disk behaves like any other: deleting the `VMDisk` removes its DataVolume, garbage collection takes the claim, and the volume and its backing storage go with it.

Imports finished by a controller older than this behaviour left their volumes on `Retain` for good, so deleting such a `VMDisk` leaves a `Released` volume that no tenant can see or remove. Setting `persistentVolumeReclaimPolicy` back to the class's policy on those volumes, while their `VMDisk` still exists, lets them clean up like the rest; on one already `Released`, the same patch makes the provisioner delete it.
