...
One behaviour change reaches clusters that never set this field. A hostname a route claimed used to earn an HTTPS-terminate listener and an ACME certificate even when a passthrough listener already held that hostname. On port 443 that put two listeners on one port and hostname: Gateway API admits the object, because its uniqueness rule counts the protocol too, and then marks both listeners `Conflicted` so neither serves — while the certificate order went to a hostname whose backend presents its own. A hostname held by a passthrough listener now produces neither, and a hostname only a `TLSRoute` claims produces neither. On upgrade, a cluster whose platform `TLSRoute`s are attached (`cozystack-api`, `vm-exportproxy`, `cdi-uploadproxy`) sheds those terminate listeners and the controller garbage-collects the certificates that went with them; they were `Conflicted` and served nothing, so the visible effect is a shorter listener list and one less certificate.
