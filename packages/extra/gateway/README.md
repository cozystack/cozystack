...
One behaviour change reaches clusters that never set this field. A hostname a route claimed used to earn an HTTPS-terminate listener and an ACME certificate even when a passthrough listener already held that hostname, and the certificate went to a hostname whose backend presents its own. On port 443 that also put two listeners on one port and hostname — Gateway API admits the pair, because its listener uniqueness rule counts the protocol as well as the port and hostname, and its compatibility rules then call for both to report `Conflicted`. A hostname a passthrough listener holds now produces neither listener nor certificate, and neither does a hostname only a `TLSRoute` claims. The shipped `tlsPassthroughServices` default is `api`, `vm-exportproxy` and `cdi-uploadproxy`, each with a platform `TLSRoute` attached, so a stock Gateway-enabled cluster sheds three terminate listeners on upgrade and the controller garbage-collects the three certificates that went with them. Read the Gateway's listener conditions before upgrading if you want to know what those listeners were doing on your cluster rather than what the spec says they should have been doing.
