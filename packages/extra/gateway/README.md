...
One behaviour change reaches clusters that never set this field. A hostname a route claimed used to earn an HTTPS-terminate listener and an ACME certificate even when a passthrough listener already held that hostname, and the certificate went to a hostname whose backend presents its own. On port 443 that also put two listeners on one port and hostname — Gateway API admits the pair, because its listener uniqueness rule counts the protocol as well as the port and hostname, and its compatibility rules then call for both to report `Conflicted`. In HTTP-01 mode a hostname a passthrough listener holds now produces neither listener nor certificate, and neither does a hostname only a `TLSRoute` claims. The other two cert modes are unaffected, because neither renders per-hostname listeners at all: the wildcard `https` and `https-apex` listeners still cover every name under the apex, passthrough hostnames included, so nothing changes and nothing is shed there. On HTTP-01 the shipped `tlsPassthroughServices` default is `api`, `vm-exportproxy` and `cdi-uploadproxy`, each with a platform `TLSRoute` attached, so a stock Gateway-enabled cluster sheds three terminate listeners on upgrade and the controller garbage-collects the three certificates that went with them. Read the Gateway's listener conditions before upgrading if you want to know what those listeners were doing on your cluster rather than what the spec says they should have been doing.

Schema constraints tightened alongside this field reach objects that already exist, and not all of them the same way. `apex` must now be a lowercase hostname of at most 253 characters, and each `tlsPassthroughServices` entry a DNS-1123 label: the apiserver ratchets constraints of that kind, so a stored value that violates one still passes any write leaving it unchanged, and the rule bites only once the value itself changes. Uniqueness of `tlsPassthroughServices` entries is declared as a list type rather than a validation, and list-type errors are never ratcheted, so an object carrying a repeated entry is refused on every write until the duplicate goes — including a write that changes something else entirely. The shipped defaults and the chart's own `fail` on an empty host conform to all of it, so an upgrade is a no-op in practice.
