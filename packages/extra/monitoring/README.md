...
| `alerta.alerts.email`                     | Configuration for email alerts.                                                                                                            | `object`   | `{}`    |
...
| `alerta.alerts.email.secretName`          | Name of an existing Secret in the release namespace holding the SMTP password under the `password` key. Required when authUsername is set. | `string`   | `""`    |
| `alerta.alerts.email.repeatInterval`      | Minimum time before a still-firing alert is emailed again.                                                                                 | `string`   | `12h`   |
