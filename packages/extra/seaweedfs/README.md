...
| `volume.maxVolumes`                               | Maximum number of logical volumes per data directory (the volume server's -max). 0 auto-computes from free disk, which reserves space for every existing sub-limit volume to grow to volumeSizeLimitMB — once that reservation exceeds free disk, placement-shaped operations (replica repair, a new bucket's first volume) fail. Set an explicit count on long-lived stores; -minFreeSpacePercent remains the disk guard. | `int`               | `0`        |
