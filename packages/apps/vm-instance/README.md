...
| `terminationGracePeriodSeconds` | Seconds the guest is given to shut down cleanly on an ACPI powerdown before the domain is destroyed. Applies to every shutdown path: an explicit stop, a node drain, an eviction, a workload update.          | `int`      | `30`        |
