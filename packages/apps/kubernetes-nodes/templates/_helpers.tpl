...
Kept in lockstep with the parent kubernetes chart's copy — both charts emit the
worker machine config and must agree on it. Not literally identical: the fail
messages say "pool" where the parent says "nodeGroup", and the comments differ.
What is identical is the emitted output, which tests/render-parity.sh proves by
comparing the machine config the two talos-reconcile Jobs apply. The duplication is the
...
{{-         fail (printf "pool %s: invalid kernelModules parameter %q on module %q — must not contain $, a backtick, a backslash, quotes or whitespace (e.g. NVreg_EnableGpuFirmware=1, or the semicolon-separated NVreg_RegistryDwords=PowerMizerEnable=0x1;PerfLevelSrc=0x2222)" $groupName $param $name) -}}
{{-       end -}}
...
Kept in lockstep with the parent kubernetes chart's copy, for the same reason as
kubernetes.kernelModules: both charts render the boot disk image and the
installer image. The fail messages and comments differ; the emitted output does
not, and tests/render-parity.sh compares both objects.
...
{{- fail (printf "pool %s: invalid schematicID %q — must not contain $, a backtick, a backslash, quotes or whitespace (it is interpolated into a machine config applied through a shell heredoc); a 64-hex image-factory digest and a mirror path such as gpu/nvidia-open are both fine" .groupName $schematicID) -}}
{{- end -}}
{{- $schematicID -}}
{{- end }}
