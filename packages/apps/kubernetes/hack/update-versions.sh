...
if grep -q "^## @enum {string} Version" "$VALUES_FILE"; then
    ...
    # Use sed to insert before "## @section Application-specific parameters"
    if [[ "$OSTYPE" == "darwin"* ]]; then
        # macOS sed
        sed -i '' -e "/^## @section Application-specific parameters/i\\
${NEW_VERSION_SECTION}\\
" "$VALUES_FILE"
    else
        # Linux sed
...
if grep -q "^KUBERNETES_VERSION" "$MAKEFILE"; then
    # Update existing KUBERNETES_VERSION line
    if [[ "$OSTYPE" == "darwin"* ]]; then
        # macOS
        sed -i '' "s|^KUBERNETES_VERSION = .*|KUBERNETES_VERSION = ${KUBERNETES_MAJOR_MINOR_FOR_MAKEFILE}|" "$MAKEFILE"
    else
        # Linux
        sed -i "s|^KUBERNETES_VERSION = .*|KUBERNETES_VERSION = ${KUBERNETES_MAJOR_MINOR_FOR_MAKEFILE}|" "$MAKEFILE"
    fi
