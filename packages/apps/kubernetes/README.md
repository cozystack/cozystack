...
| `addons.cilium`                               | Cilium CNI plugin. Always installed: it is the tenant cluster's CNI and has no enable switch, so this section only overrides its Helm values.                                        | `object`   | `{}`      |
...
| `addons.verticalPodAutoscaler`                | Vertical Pod Autoscaler. Has no enable switch of its own: it is installed and removed together with addons.monitoringAgents.enabled, so this section only overrides its Helm values. | `object`   | `{}`      |
...
| `addons.coredns`                              | CoreDNS addon. Always installed: it is the tenant cluster's DNS and has no enable switch, so this section only overrides its Helm values.                                            | `object`   | `{}`      |
