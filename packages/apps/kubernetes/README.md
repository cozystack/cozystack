...
| `nodeGroups[name].affinity`                  | Pod affinity applied to worker VMI virt-launcher pods (standard Kubernetes corev1.Affinity). Use podAntiAffinity with topologyKey `kubernetes.io/hostname` to spread worker VMIs across different hypervisor hosts.  | `object`            | `{}`        |
| `nodeGroups[name].topologySpreadConstraints` | Pod topology spread constraints applied to worker VMI virt-launcher pods (standard Kubernetes corev1.TopologySpreadConstraint). When set, takes precedence over any cluster-wide globalAppTopologySpreadConstraints. | `[]object`          | `[]`        |
