...
| `addons.ouroboros.acknowledgeUncleanRemoval`  | Defence-in-depth gate for the rare case where the chart's pre-delete cleanup hook did not land (tenant unreachable, hook timed out, manual `kubectl delete hr` bypassed helm). Set true after manually verifying the tenant kube-system/coredns-custom ConfigMap is clean (see docs). The normal disable path is fully automatic. | `bool`     | `false`   |
