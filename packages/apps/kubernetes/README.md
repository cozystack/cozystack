...
- **`ephemeralStorage` deprecated, silently ignored**: The `nodeGroups[name].ephemeralStorage` field has been renamed to `nodeGroups[name].diskSize`. Setting `ephemeralStorage` no longer blocks upgrades — Helm rendering succeeds — but the field is silently dropped and does **not** preserve the disk size. The operator fills `diskSize` from the schema default (`20Gi`) before the chart runs, so any `ephemeralStorage` value is unreachable. **Any cluster that previously set a non-default `ephemeralStorage` (e.g. `50Gi`) MUST explicitly set `diskSize` to the same value before upgrading**; otherwise the next reconciliation will provision `disk-kubelet` PVCs at `20Gi` and existing nodes may hit disk pressure or `ImagePullBackOff`. Existing VMs are automatically rolling-updated via CAPI when new values are applied. State persists across same-VM reboots; VM replacement by CAPI (e.g. MachineHealthCheck remediation) provisions a fresh PVC.
