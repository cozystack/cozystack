...
| `nodeGroups[name].resources`                    | Explicit CPU and memory for each worker node, as an alternative to `instanceType` sizing. Optional: when omitted, the node is sized by `instanceType`. Use one or the other — combining a non-empty `instanceType` with explicit `resources` is rejected at render time, because instanceType CPU/memory cannot be overridden.                                             | `object`            | `{}`        |
