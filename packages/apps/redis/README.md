...
Certificate renewal needs a restart, and only part of it is automated. The leaf certificate is valid for a year and cert-manager renews it 30 days before expiry, but Redis and Sentinel read `tls-cert-file` and `tls-key-file` once at startup and never re-read them. When TLS is on, the release annotates the RedisFailover with `secret.reloader.stakater.com/reload`, which the operator copies onto the Redis StatefulSet — where the stakater reloader, if installed, rolls the Redis pods after the Secret changes.
