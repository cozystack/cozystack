...
Certificate renewal needs a restart. The leaf certificate is valid for a year and cert-manager renews it 30 days before expiry, but Redis and Sentinel read `tls-cert-file` and `tls-key-file` once at startup and never re-read them, and neither the operator nor the chart restarts the pods when the Secret changes. After a renewal the pods keep serving the previous certificate from memory until something restarts them; if nothing does before the old certificate expires, verifying clients start failing. Restart the Redis and Sentinel pods after a renewal, or on a schedule shorter than the 30-day `renewBefore` window.

Automating this needs an operator change rather than a chart one. The platform ships stakater reloader, which acts on annotations set on the workload, but the RedisFailover CRD only exposes `podAnnotations`, and the operator writes those onto the pod template where reloader does not look. The Redis StatefulSet does inherit the RedisFailover's own `metadata.annotations`, so that half could be wired today; the Sentinel Deployment is built with no annotations field at all, so it cannot. Wiring only the reachable half would roll Redis while leaving Sentinel on an expired certificate, which is why nothing is wired here.
