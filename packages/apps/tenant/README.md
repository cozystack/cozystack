...
| `computePlane`    | Name of a ComputePlane profile to provision a single-tenant, Cozystack-managed cluster for untrusted-code workloads (empty disables it). Applications with placement: ComputePlane are routed onto it. See design-proposals/compute-plane in cozystack/community. | `string`              | `""`    |
