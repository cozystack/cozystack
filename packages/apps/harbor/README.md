...
| `managementWhitelist` | IP addresses or CIDR ranges allowed to reach the Harbor web portal and API. Only `/v2/` and `/service/token` stay open to any address, which is what `docker login`, image pulls and pushes use; every other path on the Harbor host is restricted. Empty allows every address. Requires Ingress publishing; rejected when the tenant publishes through Gateway API. | `[]string` | `[]`  |
