...
- **Management-CIDR mismatch diagnostic.** The chart's `managementCIDR` and the controller's management rule now both derive from the `ipv4-pod-cidr` key of the cozy-system/cozystack ConfigMap, with the same platform-default fallback, so a cluster with a custom `networking.podCIDR` needs neither set by hand and the two can no longer drift apart on their own. An operator who overrides `--management-cidr` can still point it somewhere the guest was never seeded with, and that mismatch has no dedicated diagnostic: the controller firewalls itself out of the guest management API and the operator sees an opaque `ConfigureFailed` push timeout with nothing in it pointing at `managementCIDR`. Comparing the controller's own pod IP against the effective CIDR and naming both in a Warning Event is what remains of this item.
