...
| 8 | Negative-security acceptance suite — authored, NOT yet executed (undeclared source / other tenant / node / API / metadata / mgmt-API / world dropped; declared source → tenant dest passes, source preserved) | deferred-to-empirical (the e2e gate; the suite is live in CI as `chainsaw-test.yaml`, not parked, and `hack/select-e2e_test.bats` pins it live — but it still has **zero executed evidence of a packet drop**, because a live run needs the published appliance image) |
| 9 | helm-unittest + Go unit + Chainsaw e2e (two-VM) green in CI | done (helm-unittest + Go unit) / deferred-to-empirical (the Chainsaw suite is wired into CI, but the live two-VM run needs the VyOS appliance image to be published) |
