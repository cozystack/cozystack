...
| Name                          | Description                                                                                                                                                                                                                                | Type       | Value   |
| ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------- | ------- |
| `remoteCIDRs`                 | Remote networks reachable over the tunnel. Must be disjoint from cluster pod/service/join networks, live node addresses, and allocated LoadBalancer/external service addresses (validated at admission and by the controller). At most 16. | `[]string` | `[]`    |
...
| Name            | Description                                                                                                                                                            | Type     | Value |
| --------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ----- |
| `cloudInitSeed` | Opaque seed mixed into the VM firmware UUID. Change it to force the first-boot seed to be applied again; clear it to preserve an existing VM's UUID across re-renders. | `string` | `""`  |
